    ...
    return __async(this, null, function* () {
      try {
        ...
        if (this.tools && Object.keys(this.tools).length > 0) {
          const customTools = Object.entries(this.tools).map(([name14, tool]) => {
          ...
          if (item.name === "computer") {
            ...
            if (this.tools && item.name in this.tools) {
              try {
                ...
                const result = yield tool.execute(item.input, {
                  toolCallId: item.id,
                  messages: []
                });
...
var StagehandOperatorHandler = class {
  constructor(stagehandPage, logger, llmClient, mcpTools, logInferenceToFile = false) {
    this.stagehandPage = stagehandPage;
    this.logger = logger;
    this.llmClient = llmClient;
    this.messages = [];
    ...
    const stagehandTools = {
      act: {
        description: "Perform an action on the page. Use this to interact with elements like clicking buttons, typing text, or navigating.",
        parameters: import_zod.z.object({
          action: import_zod.z.string().describe(
        ...
        execute: (args) => __async(this, null, function* () {
          const [playwrightArguments] = yield this.stagehandPage.page.observe(
            args.action
          );
          yield this.stagehandPage.page.act(playwrightArguments);
          return {
            success: true,
            action: args.action,
      ...
      extract: {
        description: "Extract data from the page. Use this to get information like text, links, or structured data.",
        parameters: import_zod.z.object({
          instruction: import_zod.z.string().optional().describe(
      ...
      goto: {
        ...
        parameters: import_zod.z.object({
          url: import_zod.z.string().describe("The URL to navigate to. e.g. 'https://www.google.com'")
        }),
        execute: (args) => __async(this, null, function* () {
          yield this.stagehandPage.page.goto(args.url, { waitUntil: "load" });
          return {
            success: true,
            url: args.url,
      ...
      wait: {
        ...
        parameters: import_zod.z.object({
          milliseconds: import_zod.z.number().describe("The amount of time to wait in milliseconds")
        }),
        execute: (args) => __async(this, null, function* () {
          yield this.stagehandPage.page.waitForTimeout(args.milliseconds);
          return {
            success: true,
      ...
      navback: {
        description: "Navigate back to the previous page. Do not use if you are already on the first page.",
        parameters: import_zod.z.object({}),
      ...
      close: {
        description: "Close the task and finish execution. Use this when the task is complete or cannot be achieved.",
        parameters: import_zod.z.object({
          reason: import_zod.z.string().describe("The reason for closing the task"),
          success: import_zod.z.boolean().describe("Whether the task was completed successfully")
        }),
        execute: (args) => __async(this, null, function* () {
          return {
            success: true,
    ...
    this.allTools = __spreadValues(__spreadValues({}, stagehandTools), mcpTools);
  }
...
${messageText}`;
  ...
  getNextStep(currentStep) {
    return __async(this, null, function* () {
      ...
      const toolsArray = Object.entries(this.allTools).map(([name14, tool]) => ({
        type: "function",
        name: name14,
        description: tool.description,
        parameters: tool.parameters
      ...
      const startTime = Date.now();
      const response = yield this.llmClient.createChatCompletion({
        options: {
          messages: this.messages,
          temperature: this.getTemperatureForModel(),
          tools: toolsArray,
          tool_choice: "required",
          // Force tool usage since operator expects tool calls
          requestId
        },
        logger: this.logger
      });
      const endTime = Date.now();
      const toolCalls = ((_d = (_c = (_b = (_a15 = response.choices) == null ? void 0 : _a15[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.tool_calls) == null ? void 0 : _d.map((tc) => ({
        toolCallId: tc.id,
      ...
      const responseText = ((_g = (_f = (_e = response.choices) == null ? void 0 : _e[0]) == null ? void 0 : _f.message) == null ? void 0 : _g.content) || "";
      if (this.logInferenceToFile) {
        const { fileName } = writeTimestampedTxtFile(
          "agent_summary",
          "agent_response",
        ...
        appendSummary("agent", {
          ...
          prompt_tokens: (_i = (_h = response.usage) == null ? void 0 : _h.prompt_tokens) != null ? _i : 0,
          completion_tokens: (_k = (_j = response.usage) == null ? void 0 : _j.completion_tokens) != null ? _k : 0,
          inference_time_ms: endTime - startTime
        });
      }
      this.logger({
        category: "agent",
        message: `LLM response - ${(toolCalls == null ? void 0 : toolCalls.length) || 0}, Text: "${responseText || "none"}"`,
        level: 1
      });
      if (toolCalls && toolCalls.length > 0) {
        ...
        for (const toolCall of toolCalls) {
          ...
          try {
            const result = yield tool.execute(toolArgs, {
        ...
        for (let i = 0; i < toolCalls.length; i++) {
          ...
          const resultText = toolResult && toolResult.trim() ? `Tool "${toolCall.toolName}" result:
${toolResult}` : `Tool "${toolCall.toolName}" completed with no output.`;
          this.messages.push({
            role: "user",
            content: [
              {
  ...
  getSummary(goal) {
    return __async(this, null, function* () {
      ...
      const startTime = Date.now();
      const { data: response, usage } = yield this.llmClient.createChatCompletion({
        options: {
          temperature: this.getTemperatureForModel(),
          messages: summaryMessages,
          response_model: {
      ...
      if (this.logInferenceToFile) {
        ...
        appendSummary("agent", {
          ...
          LLM_input_file: callFile,
          LLM_output_file: fileName,
          prompt_tokens: (_a15 = usage == null ? void 0 : usage.prompt_tokens) != null ? _a15 : 0,
          completion_tokens: (_b = usage == null ? void 0 : usage.completion_tokens) != null ? _b : 0,
          inference_time_ms: endTime - startTime
        });
      }
...
var openaiErrorDataSchema = import_zod4.z.object({
  error: import_zod4.z.object({
    ...
    param: import_zod4.z.any().nullish(),
    code: import_zod4.z.union([import_zod4.z.string(), import_zod4.z.number()]).nullish()
  })
});
  ...
  choices: import_zod3.z.array(
    import_zod3.z.object({
      message: import_zod3.z.object({
        ...
        function_call: import_zod3.z.object({
  ...
  choices: import_zod5.z.array(
    import_zod5.z.object({
      ...
      logprobs: import_zod5.z.object({
        ...
        top_logprobs: import_zod5.z.array(import_zod5.z.record(import_zod5.z.string(), import_zod5.z.number())).nullable()
      }).nullish()
    })
  ),
            ...
            output: import_zod9.z.array(
              import_zod9.z.discriminatedUnion("type", [
                ...
                import_zod9.z.object({
                  ...
                  summary: import_zod9.z.array(
                    import_zod9.z.object({
                      type: import_zod9.z.literal("summary_text"),
                      text: import_zod9.z.string()
                    })
                  )
                })
...
var responseReasoningSummaryTextDeltaSchema = import_zod9.z.object({
  type: import_zod9.z.literal("response.reasoning_summary_text.delta"),
  item_id: import_zod9.z.string(),
  output_index: import_zod9.z.number(),
  summary_index: import_zod9.z.number(),
  delta: import_zod9.z.string()
});
...
var openaiResponsesProviderOptionsSchema = import_zod9.z.object({
  ...
  reasoningSummary: import_zod9.z.string().nullish()
});
var WebSearchPreviewParameters = import_zod10.z.object({});
...
var anthropicErrorDataSchema = import_zod13.z.object({
  type: import_zod13.z.literal("error"),
  error: import_zod13.z.object({
...
var anthropicMessagesResponseSchema = import_zod12.z.object({
  type: import_zod12.z.literal("message"),
  id: import_zod12.z.string().nullish(),
  model: import_zod12.z.string().nullish(),
  content: import_zod12.z.array(
    import_zod12.z.discriminatedUnion("type", [
      ...
      import_zod12.z.object({
        type: import_zod12.z.literal("thinking"),
        thinking: import_zod12.z.string(),
        signature: import_zod12.z.string()
      }),
      import_zod12.z.object({
        type: import_zod12.z.literal("redacted_thinking"),
        data: import_zod12.z.string()
      }),
      import_zod12.z.object({
        type: import_zod12.z.literal("tool_use"),
        id: import_zod12.z.string(),
        name: import_zod12.z.string(),
        input: import_zod12.z.unknown()
      })
    ])
  ),
  stop_reason: import_zod12.z.string().nullish(),
  usage: import_zod12.z.object({
    input_tokens: import_zod12.z.number(),
    output_tokens: import_zod12.z.number(),
    cache_creation_input_tokens: import_zod12.z.number().nullish(),
    cache_read_input_tokens: import_zod12.z.number().nullish()
  })
});
var anthropicMessagesChunkSchema = import_zod12.z.discriminatedUnion("type", [
  import_zod12.z.object({
    type: import_zod12.z.literal("message_start"),
  ...
  import_zod12.z.object({
    type: import_zod12.z.literal("content_block_start"),
    index: import_zod12.z.number(),
    content_block: import_zod12.z.discriminatedUnion("type", [
      import_zod12.z.object({
        type: import_zod12.z.literal("text"),
        text: import_zod12.z.string()
      }),
  ...
  import_zod12.z.object({
    type: import_zod12.z.literal("content_block_delta"),
    index: import_zod12.z.number(),
    delta: import_zod12.z.discriminatedUnion("type", [
      import_zod12.z.object({
        type: import_zod12.z.literal("input_json_delta"),
        partial_json: import_zod12.z.string()
      }),
      import_zod12.z.object({
        type: import_zod12.z.literal("text_delta"),
        text: import_zod12.z.string()
      }),
      import_zod12.z.object({
        type: import_zod12.z.literal("thinking_delta"),
        thinking: import_zod12.z.string()
      }),
      import_zod12.z.object({
        type: import_zod12.z.literal("signature_delta"),
        signature: import_zod12.z.string()
      })
    ])
  }),
  import_zod12.z.object({
    type: import_zod12.z.literal("content_block_stop"),
    index: import_zod12.z.number()
  }),
  import_zod12.z.object({
  ...
  import_zod12.z.object({
    type: import_zod12.z.literal("message_delta"),
    delta: import_zod12.z.object({ stop_reason: import_zod12.z.string().nullish() }),
    usage: import_zod12.z.object({ output_tokens: import_zod12.z.number() })
  }),
  import_zod12.z.object({
    type: import_zod12.z.literal("message_stop")
  }),
  import_zod12.z.object({
    type: import_zod12.z.literal("ping")
  })
]);
var anthropicProviderOptionsSchema = import_zod12.z.object({
  thinking: import_zod12.z.object({
    type: import_zod12.z.union([import_zod12.z.literal("enabled"), import_zod12.z.literal("disabled")]),
    budgetTokens: import_zod12.z.number().optional()
  }).optional()
});
var Bash20241022Parameters = import_zod14.z.object({
  command: import_zod14.z.string(),
  restart: import_zod14.z.boolean().optional()
});
...
var Bash20250124Parameters = import_zod14.z.object({
  command: import_zod14.z.string(),
  restart: import_zod14.z.boolean().optional()
});
...
var TextEditor20241022Parameters = import_zod14.z.object({
  command: import_zod14.z.enum(["view", "create", "str_replace", "insert", "undo_edit"]),
  path: import_zod14.z.string(),
  file_text: import_zod14.z.string().optional(),
  insert_line: import_zod14.z.number().int().optional(),
  new_str: import_zod14.z.string().optional(),
  old_str: import_zod14.z.string().optional(),
  view_range: import_zod14.z.array(import_zod14.z.number().int()).optional()
});
...
var TextEditor20250124Parameters = import_zod14.z.object({
  command: import_zod14.z.enum(["view", "create", "str_replace", "insert", "undo_edit"]),
  path: import_zod14.z.string(),
  file_text: import_zod14.z.string().optional(),
  insert_line: import_zod14.z.number().int().optional(),
...
var Computer20241022Parameters = import_zod14.z.object({
  action: import_zod14.z.enum([
    "key",
    "type",
    "mouse_move",
    "left_click",
  ...
  coordinate: import_zod14.z.array(import_zod14.z.number().int()).optional(),
...
var Computer20250124Parameters = import_zod14.z.object({
  action: import_zod14.z.enum([
    "key",
    "hold_key",
    "type",
  ...
  coordinate: import_zod14.z.tuple([import_zod14.z.number().int(), import_zod14.z.number().int()]).optional(),
  duration: import_zod14.z.number().optional(),
  scroll_amount: import_zod14.z.number().optional(),
  scroll_direction: import_zod14.z.enum(["up", "down", "left", "right"]).optional(),
  start_coordinate: import_zod14.z.tuple([import_zod14.z.number().int(), import_zod14.z.number().int()]).optional(),
  text: import_zod14.z.string().optional()
});
...
var googleErrorDataSchema = import_zod16.z.object({
  error: import_zod16.z.object({
    code: import_zod16.z.number().nullable(),
...
var contentSchema = import_zod15.z.object({
  role: import_zod15.z.string(),
  parts: import_zod15.z.array(
    import_zod15.z.union([
      import_zod15.z.object({
        text: import_zod15.z.string()
      }),
      import_zod15.z.object({
        functionCall: import_zod15.z.object({
          name: import_zod15.z.string(),
          args: import_zod15.z.unknown()
        })
      }),
      import_zod15.z.object({
        inlineData: import_zod15.z.object({
...
var groundingChunkSchema = import_zod15.z.object({
  web: import_zod15.z.object({ uri: import_zod15.z.string(), title: import_zod15.z.string() }).nullish(),
  retrievedContext: import_zod15.z.object({ uri: import_zod15.z.string(), title: import_zod15.z.string() }).nullish()
});
var groundingMetadataSchema = import_zod15.z.object({
  webSearchQueries: import_zod15.z.array(import_zod15.z.string()).nullish(),
  retrievalQueries: import_zod15.z.array(import_zod15.z.string()).nullish(),
  searchEntryPoint: import_zod15.z.object({ renderedContent: import_zod15.z.string() }).nullish(),
  groundingChunks: import_zod15.z.array(groundingChunkSchema).nullish(),
  groundingSupports: import_zod15.z.array(
    import_zod15.z.object({
      segment: import_zod15.z.object({
        startIndex: import_zod15.z.number().nullish(),
        endIndex: import_zod15.z.number().nullish(),
        text: import_zod15.z.string().nullish()
      }),
      segment_text: import_zod15.z.string().nullish(),
      groundingChunkIndices: import_zod15.z.array(import_zod15.z.number()).nullish(),
      supportChunkIndices: import_zod15.z.array(import_zod15.z.number()).nullish(),
      confidenceScores: import_zod15.z.array(import_zod15.z.number()).nullish(),
      confidenceScore: import_zod15.z.array(import_zod15.z.number()).nullish()
    })
  ).nullish(),
  retrievalMetadata: import_zod15.z.union([
    import_zod15.z.object({
      webDynamicRetrievalScore: import_zod15.z.number()
    }),
    import_zod15.z.object({})
  ]).nullish()
});
var safetyRatingSchema = import_zod15.z.object({
  category: import_zod15.z.string(),
  probability: import_zod15.z.string(),
  probabilityScore: import_zod15.z.number().nullish(),
  severity: import_zod15.z.string().nullish(),
  severityScore: import_zod15.z.number().nullish(),
  blocked: import_zod15.z.boolean().nullish()
});
var responseSchema = import_zod15.z.object({
  candidates: import_zod15.z.array(
    import_zod15.z.object({
      content: contentSchema.nullish().or(import_zod15.z.object({}).strict()),
      finishReason: import_zod15.z.string().nullish(),
      safetyRatings: import_zod15.z.array(safetyRatingSchema).nullish(),
      groundingMetadata: groundingMetadataSchema.nullish()
    })
  ),
  usageMetadata: import_zod15.z.object({
    promptTokenCount: import_zod15.z.number().nullish(),
    candidatesTokenCount: import_zod15.z.number().nullish(),
    totalTokenCount: import_zod15.z.number().nullish()
  }).nullish()
});
var chunkSchema = import_zod15.z.object({
  candidates: import_zod15.z.array(
    import_zod15.z.object({
...
var googleGenerativeAIProviderOptionsSchema = import_zod15.z.object({
  responseModalities: import_zod15.z.array(import_zod15.z.enum(["TEXT", "IMAGE"])).nullish(),
  thinkingConfig: import_zod15.z.object({
    thinkingBudget: import_zod15.z.number().nullish()
  }).nullish()
});
...
var googleGenerativeAITextEmbeddingResponseSchema = import_zod17.z.object({
  embeddings: import_zod17.z.array(import_zod17.z.object({ values: import_zod17.z.array(import_zod17.z.number()) }))
});
...
var openaiCompatibleErrorDataSchema = import_zod19.z.object({
  error: import_zod19.z.object({
    message: import_zod19.z.string(),
    // The additional information below is handled loosely to support
    // OpenAI-compatible providers that have slightly different error
    // responses:
    type: import_zod19.z.string().nullish(),
    param: import_zod19.z.any().nullish(),
    code: import_zod19.z.union([import_zod19.z.string(), import_zod19.z.number()]).nullish()
  })
});
...
var openaiCompatibleTokenUsageSchema = import_zod18.z.object({
  prompt_tokens: import_zod18.z.number().nullish(),
  completion_tokens: import_zod18.z.number().nullish(),
  prompt_tokens_details: import_zod18.z.object({
    cached_tokens: import_zod18.z.number().nullish()
  }).nullish(),
  completion_tokens_details: import_zod18.z.object({
    reasoning_tokens: import_zod18.z.number().nullish(),
    accepted_prediction_tokens: import_zod18.z.number().nullish(),
    rejected_prediction_tokens: import_zod18.z.number().nullish()
  }).nullish()
}).nullish();
var OpenAICompatibleChatResponseSchema = import_zod18.z.object({
  id: import_zod18.z.string().nullish(),
  created: import_zod18.z.number().nullish(),
  model: import_zod18.z.string().nullish(),
  choices: import_zod18.z.array(
    import_zod18.z.object({
      message: import_zod18.z.object({
        role: import_zod18.z.literal("assistant").nullish(),
        content: import_zod18.z.string().nullish(),
        reasoning_content: import_zod18.z.string().nullish(),
        tool_calls: import_zod18.z.array(
          import_zod18.z.object({
            id: import_zod18.z.string().nullish(),
            type: import_zod18.z.literal("function"),
            function: import_zod18.z.object({
              name: import_zod18.z.string(),
              arguments: import_zod18.z.string()
            })
          })
        ).nullish()
      }),
      finish_reason: import_zod18.z.string().nullish()
    })
  ),
  usage: openaiCompatibleTokenUsageSchema
});
var createOpenAICompatibleChatChunkSchema = (errorSchema) => import_zod18.z.union([
  import_zod18.z.object({
    id: import_zod18.z.string().nullish(),
    created: import_zod18.z.number().nullish(),
    model: import_zod18.z.string().nullish(),
    choices: import_zod18.z.array(
      import_zod18.z.object({
        delta: import_zod18.z.object({
          role: import_zod18.z.enum(["assistant"]).nullish(),
          content: import_zod18.z.string().nullish(),
          reasoning_content: import_zod18.z.string().nullish(),
          tool_calls: import_zod18.z.array(
            import_zod18.z.object({
              ...
              id: import_zod18.z.string().nullish(),
              type: import_zod18.z.literal("function").nullish(),
              function: import_zod18.z.object({
                name: import_zod18.z.string().nullish(),
                arguments: import_zod18.z.string().nullish()
              })
            })
          ).nullish()
        }).nullish(),
...
var openaiCompatibleCompletionResponseSchema = import_zod20.z.object({
  id: import_zod20.z.string().nullish(),
  created: import_zod20.z.number().nullish(),
  model: import_zod20.z.string().nullish(),
  choices: import_zod20.z.array(
    import_zod20.z.object({
      text: import_zod20.z.string(),
      finish_reason: import_zod20.z.string()
    })
  ),
  usage: import_zod20.z.object({
    prompt_tokens: import_zod20.z.number(),
    completion_tokens: import_zod20.z.number()
  }).nullish()
});
var createOpenAICompatibleCompletionChunkSchema = (errorSchema) => import_zod20.z.union([
  import_zod20.z.object({
    id: import_zod20.z.string().nullish(),
    created: import_zod20.z.number().nullish(),
    model: import_zod20.z.string().nullish(),
    choices: import_zod20.z.array(
      import_zod20.z.object({
        text: import_zod20.z.string(),
        finish_reason: import_zod20.z.string().nullish(),
        index: import_zod20.z.number()
      })
    ),
    usage: import_zod20.z.object({
      prompt_tokens: import_zod20.z.number(),
...
var openaiTextEmbeddingResponseSchema2 = import_zod21.z.object({
  data: import_zod21.z.array(import_zod21.z.object({ embedding: import_zod21.z.array(import_zod21.z.number()) })),
  usage: import_zod21.z.object({ prompt_tokens: import_zod21.z.number() }).nullish()
});
...
var openaiCompatibleImageResponseSchema = import_zod22.z.object({
  data: import_zod22.z.array(import_zod22.z.object({ b64_json: import_zod22.z.string() }))
});
...
var xaiErrorSchema = import_zod23.z.object({
...
var openaiErrorDataSchema2 = import_zod25.z.object({
  error: import_zod25.z.object({
    ...
    param: import_zod25.z.any().nullish(),
    code: import_zod25.z.union([import_zod25.z.string(), import_zod25.z.number()]).nullish()
  })
});
...
var openaiTokenUsageSchema2 = import_zod24.z.object({
  prompt_tokens: import_zod24.z.number().nullish(),
  completion_tokens: import_zod24.z.number().nullish(),
  prompt_tokens_details: import_zod24.z.object({
    cached_tokens: import_zod24.z.number().nullish()
  }).nullish(),
  completion_tokens_details: import_zod24.z.object({
    reasoning_tokens: import_zod24.z.number().nullish(),
    accepted_prediction_tokens: import_zod24.z.number().nullish(),
    rejected_prediction_tokens: import_zod24.z.number().nullish()
  }).nullish()
}).nullish();
var openaiChatResponseSchema2 = import_zod24.z.object({
  id: import_zod24.z.string().nullish(),
  created: import_zod24.z.number().nullish(),
  model: import_zod24.z.string().nullish(),
  choices: import_zod24.z.array(
    import_zod24.z.object({
      message: import_zod24.z.object({
        role: import_zod24.z.literal("assistant").nullish(),
        content: import_zod24.z.string().nullish(),
      ...
      logprobs: import_zod24.z.object({
        content: import_zod24.z.array(
          import_zod24.z.object({
            token: import_zod24.z.string(),
            logprob: import_zod24.z.number(),
            top_logprobs: import_zod24.z.array(
              import_zod24.z.object({
                token: import_zod24.z.string(),
                logprob: import_zod24.z.number()
              })
      ...
      finish_reason: import_zod24.z.string().nullish()
    })
  ),
  usage: openaiTokenUsageSchema2
});
var openaiChatChunkSchema2 = import_zod24.z.union([
  import_zod24.z.object({
    id: import_zod24.z.string().nullish(),
    created: import_zod24.z.number().nullish(),
    model: import_zod24.z.string().nullish(),
    choices: import_zod24.z.array(
      import_zod24.z.object({
        delta: import_zod24.z.object({
          role: import_zod24.z.enum(["assistant"]).nullish(),
          content: import_zod24.z.string().nullish(),
          function_call: import_zod24.z.object({
          ...
          tool_calls: import_zod24.z.array(
            import_zod24.z.object({
              index: import_zod24.z.number(),
              id: import_zod24.z.string().nullish(),
              type: import_zod24.z.literal("function").nullish(),
              function: import_zod24.z.object({
                name: import_zod24.z.string().nullish(),
                arguments: import_zod24.z.string().nullish()
              })
            })
          ).nullish()
        }).nullish(),
...
var openaiCompletionResponseSchema2 = import_zod26.z.object({
  id: import_zod26.z.string().nullish(),
  created: import_zod26.z.number().nullish(),
  model: import_zod26.z.string().nullish(),
  choices: import_zod26.z.array(
    import_zod26.z.object({
      text: import_zod26.z.string(),
      finish_reason: import_zod26.z.string(),
      logprobs: import_zod26.z.object({
        tokens: import_zod26.z.array(import_zod26.z.string()),
        token_logprobs: import_zod26.z.array(import_zod26.z.number()),
        top_logprobs: import_zod26.z.array(import_zod26.z.record(import_zod26.z.string(), import_zod26.z.number())).nullable()
      }).nullish()
    })
  ),
  usage: import_zod26.z.object({
    prompt_tokens: import_zod26.z.number(),
    completion_tokens: import_zod26.z.number()
  })
});
var openaiCompletionChunkSchema2 = import_zod26.z.union([
  import_zod26.z.object({
    id: import_zod26.z.string().nullish(),
    created: import_zod26.z.number().nullish(),
    model: import_zod26.z.string().nullish(),
    choices: import_zod26.z.array(
      import_zod26.z.object({
        text: import_zod26.z.string(),
        finish_reason: import_zod26.z.string().nullish(),
...
var openaiTextEmbeddingResponseSchema3 = import_zod27.z.object({
  data: import_zod27.z.array(import_zod27.z.object({ embedding: import_zod27.z.array(import_zod27.z.number()) })),
  usage: import_zod27.z.object({ prompt_tokens: import_zod27.z.number() }).nullish()
});
...
var openaiImageResponseSchema2 = import_zod28.z.object({
  data: import_zod28.z.array(import_zod28.z.object({ b64_json: import_zod28.z.string() }))
});
var openAIProviderOptionsSchema2 = import_zod29.z.object({
  include: import_zod29.z.array(import_zod29.z.string()).nullish(),
  language: import_zod29.z.string().nullish(),
  prompt: import_zod29.z.string().nullish(),
  temperature: import_zod29.z.number().min(0).max(1).nullish().default(0),
  timestampGranularities: import_zod29.z.array(import_zod29.z.enum(["word", "segment"])).nullish().default(["segment"])
});
var languageMap2 = {
  afrikaans: "af",
  arabic: "ar",
...
var openaiTranscriptionResponseSchema2 = import_zod29.z.object({
  text: import_zod29.z.string(),
  language: import_zod29.z.string().nullish(),
  duration: import_zod29.z.number().nullish(),
  words: import_zod29.z.array(
    import_zod29.z.object({
      word: import_zod29.z.string(),
      start: import_zod29.z.number(),
      end: import_zod29.z.number()
    })
  ).nullish()
});
var OpenAIProviderOptionsSchema2 = import_zod30.z.object({
  instructions: import_zod30.z.string().nullish(),
  speed: import_zod30.z.number().min(0.25).max(4).default(1).nullish()
});
      ...
      const {
        ...
        successfulResponseHandler: createJsonResponseHandler(
          import_zod31.z.object({
            ...
            created_at: import_zod31.z.number(),
            model: import_zod31.z.string(),
            output: import_zod31.z.array(
              import_zod31.z.discriminatedUnion("type", [
                import_zod31.z.object({
                  type: import_zod31.z.literal("message"),
                  role: import_zod31.z.literal("assistant"),
                  content: import_zod31.z.array(
                    import_zod31.z.object({
                      type: import_zod31.z.literal("output_text"),
                      text: import_zod31.z.string(),
                      annotations: import_zod31.z.array(
                        import_zod31.z.object({
                          type: import_zod31.z.literal("url_citation"),
                          start_index: import_zod31.z.number(),
                          end_index: import_zod31.z.number(),
                ...
                import_zod31.z.object({
                  type: import_zod31.z.literal("function_call"),
                  call_id: import_zod31.z.string(),
                  name: import_zod31.z.string(),
                  arguments: import_zod31.z.string()
                }),
                import_zod31.z.object({
                  type: import_zod31.z.literal("web_search_call")
                }),
                import_zod31.z.object({
                  type: import_zod31.z.literal("computer_call")
                }),
                import_zod31.z.object({
                  type: import_zod31.z.literal("reasoning"),
                  summary: import_zod31.z.array(
                    import_zod31.z.object({
                      type: import_zod31.z.literal("summary_text"),
                      text: import_zod31.z.string()
                    })
                  )
                })
            ...
            incomplete_details: import_zod31.z.object({ reason: import_zod31.z.string() }).nullable(),
            usage: usageSchema2
          })
        ),
        abortSignal: options.abortSignal,
...
var usageSchema2 = import_zod31.z.object({
  input_tokens: import_zod31.z.number(),
  input_tokens_details: import_zod31.z.object({ cached_tokens: import_zod31.z.number().nullish() }).nullish(),
  output_tokens: import_zod31.z.number(),
  output_tokens_details: import_zod31.z.object({ reasoning_tokens: import_zod31.z.number().nullish() }).nullish()
});
var textDeltaChunkSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.output_text.delta"),
  delta: import_zod31.z.string()
});
var responseFinishedChunkSchema2 = import_zod31.z.object({
  type: import_zod31.z.enum(["response.completed", "response.incomplete"]),
  response: import_zod31.z.object({
    incomplete_details: import_zod31.z.object({ reason: import_zod31.z.string() }).nullish(),
    usage: usageSchema2
  })
});
var responseCreatedChunkSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.created"),
  response: import_zod31.z.object({
    id: import_zod31.z.string(),
    created_at: import_zod31.z.number(),
    model: import_zod31.z.string()
...
var responseOutputItemDoneSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.output_item.done"),
  output_index: import_zod31.z.number(),
  item: import_zod31.z.discriminatedUnion("type", [
    import_zod31.z.object({
      type: import_zod31.z.literal("message")
    }),
    import_zod31.z.object({
      ...
      name: import_zod31.z.string(),
      arguments: import_zod31.z.string(),
      status: import_zod31.z.literal("completed")
    })
  ])
});
var responseFunctionCallArgumentsDeltaSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.function_call_arguments.delta"),
  item_id: import_zod31.z.string(),
  output_index: import_zod31.z.number(),
  delta: import_zod31.z.string()
});
var responseOutputItemAddedSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.output_item.added"),
  output_index: import_zod31.z.number(),
  item: import_zod31.z.discriminatedUnion("type", [
    import_zod31.z.object({
      type: import_zod31.z.literal("message")
...
var responseAnnotationAddedSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.output_text.annotation.added"),
  annotation: import_zod31.z.object({
    type: import_zod31.z.literal("url_citation"),
    url: import_zod31.z.string(),
    title: import_zod31.z.string()
  })
});
var responseReasoningSummaryTextDeltaSchema2 = import_zod31.z.object({
  type: import_zod31.z.literal("response.reasoning_summary_text.delta"),
  item_id: import_zod31.z.string(),
  output_index: import_zod31.z.number(),
  summary_index: import_zod31.z.number(),
  delta: import_zod31.z.string()
});
var openaiResponsesChunkSchema2 = import_zod31.z.union([
  ...
  import_zod31.z.object({ type: import_zod31.z.string() }).passthrough()
  // fallback for unknown chunks
]);
...
var openaiResponsesProviderOptionsSchema2 = import_zod31.z.object({
  metadata: import_zod31.z.any().nullish(),
  parallelToolCalls: import_zod31.z.boolean().nullish(),
  previousResponseId: import_zod31.z.string().nullish(),
  store: import_zod31.z.boolean().nullish(),
  user: import_zod31.z.string().nullish(),
  reasoningEffort: import_zod31.z.string().nullish(),
  strictSchemas: import_zod31.z.boolean().nullish(),
  instructions: import_zod31.z.string().nullish(),
  reasoningSummary: import_zod31.z.string().nullish()
});
...
var groqErrorDataSchema = import_zod33.z.object({
  error: import_zod33.z.object({
...
var GroqChatLanguageModel = class {
  ...
  getArgs({
    ...
    const groqOptions = parseProviderOptions({
      provider: "groq",
      providerOptions: providerMetadata,
      schema: import_zod32.z.object({
        reasoningFormat: import_zod32.z.enum(["parsed", "raw", "hidden"]).nullish()
      })
    });
    const baseArgs = {
      // model id:
...
var groqChatResponseSchema = import_zod32.z.object({
  ...
  choices: import_zod32.z.array(
    import_zod32.z.object({
      message: import_zod32.z.object({
        content: import_zod32.z.string().nullish(),
        reasoning: import_zod32.z.string().nullish(),
        tool_calls: import_zod32.z.array(
          import_zod32.z.object({
            id: import_zod32.z.string().nullish(),
            type: import_zod32.z.literal("function"),
            function: import_zod32.z.object({
              name: import_zod32.z.string(),
              arguments: import_zod32.z.string()
            })
          })
        ).nullish()
      }),
  ...
  usage: import_zod32.z.object({
    prompt_tokens: import_zod32.z.number().nullish(),
    completion_tokens: import_zod32.z.number().nullish()
  }).nullish()
});
var groqChatChunkSchema = import_zod32.z.union([
  import_zod32.z.object({
    id: import_zod32.z.string().nullish(),
    created: import_zod32.z.number().nullish(),
    model: import_zod32.z.string().nullish(),
        ...
        delta: import_zod32.z.object({
          ...
          tool_calls: import_zod32.z.array(
            import_zod32.z.object({
              index: import_zod32.z.number(),
              id: import_zod32.z.string().nullish(),
              type: import_zod32.z.literal("function").optional(),
              function: import_zod32.z.object({
                name: import_zod32.z.string().nullish(),
                arguments: import_zod32.z.string().nullish()
              })
            })
          ).nullish()
        }).nullish(),
        finish_reason: import_zod32.z.string().nullable().optional(),
        index: import_zod32.z.number()
      })
    ),
    x_groq: import_zod32.z.object({
      usage: import_zod32.z.object({
        prompt_tokens: import_zod32.z.number().nullish(),
        completion_tokens: import_zod32.z.number().nullish()
      }).nullish()
...
var groqProviderOptionsSchema = import_zod34.z.object({
  language: import_zod34.z.string().nullish(),
  prompt: import_zod34.z.string().nullish(),
  responseFormat: import_zod34.z.string().nullish(),
  temperature: import_zod34.z.number().min(0).max(1).nullish(),
  timestampGranularities: import_zod34.z.array(import_zod34.z.string()).nullish()
});
...
var groqTranscriptionResponseSchema = import_zod34.z.object({
  task: import_zod34.z.string(),
  language: import_zod34.z.string(),
  duration: import_zod34.z.number(),
  text: import_zod34.z.string(),
  segments: import_zod34.z.array(
    import_zod34.z.object({
      id: import_zod34.z.number(),
      seek: import_zod34.z.number(),
      start: import_zod34.z.number(),
      end: import_zod34.z.number(),
      text: import_zod34.z.string(),
      tokens: import_zod34.z.array(import_zod34.z.number()),
      temperature: import_zod34.z.number(),
      avg_logprob: import_zod34.z.number(),
      compression_ratio: import_zod34.z.number(),
      no_speech_prob: import_zod34.z.number()
    })
  ),
  x_groq: import_zod34.z.object({
...
var cerebrasErrorSchema = import_zod35.z.object({
  message: import_zod35.z.string(),
  type: import_zod35.z.string(),
  param: import_zod35.z.string(),
  code: import_zod35.z.string()
});
...
var togetheraiImageResponseSchema = import_zod36.z.object({
  data: import_zod36.z.array(
    import_zod36.z.object({
      b64_json: import_zod36.z.string()
    })
  )
});
var togetheraiErrorSchema = import_zod36.z.object({
...
var mistralErrorDataSchema = import_zod38.z.object({
  object: import_zod38.z.literal("error"),
  message: import_zod38.z.string(),
  type: import_zod38.z.string(),
  param: import_zod38.z.string().nullable(),
  code: import_zod38.z.string().nullable()
});
...
var mistralContentSchema = import_zod37.z.union([
  import_zod37.z.string(),
  import_zod37.z.array(
    import_zod37.z.discriminatedUnion("type", [
      import_zod37.z.object({
        type: import_zod37.z.literal("text"),
        text: import_zod37.z.string()
      }),
      import_zod37.z.object({
        type: import_zod37.z.literal("image_url"),
        image_url: import_zod37.z.union([
          import_zod37.z.string(),
          import_zod37.z.object({
            url: import_zod37.z.string(),
            detail: import_zod37.z.string().nullable()
          })
        ])
      }),
      import_zod37.z.object({
        type: import_zod37.z.literal("reference"),
        reference_ids: import_zod37.z.array(import_zod37.z.number())
      })
    ])
  )
]).nullish();
var mistralChatResponseSchema = import_zod37.z.object({
  id: import_zod37.z.string().nullish(),
  created: import_zod37.z.number().nullish(),
  model: import_zod37.z.string().nullish(),
  choices: import_zod37.z.array(
    import_zod37.z.object({
      message: import_zod37.z.object({
        role: import_zod37.z.literal("assistant"),
        content: mistralContentSchema,
        tool_calls: import_zod37.z.array(
          import_zod37.z.object({
            id: import_zod37.z.string(),
            function: import_zod37.z.object({ name: import_zod37.z.string(), arguments: import_zod37.z.string() })
          })
        ).nullish()
      }),
      index: import_zod37.z.number(),
      finish_reason: import_zod37.z.string().nullish()
    })
  ),
  object: import_zod37.z.literal("chat.completion"),
  usage: import_zod37.z.object({
    prompt_tokens: import_zod37.z.number(),
    completion_tokens: import_zod37.z.number()
  })
});
var mistralChatChunkSchema = import_zod37.z.object({
  id: import_zod37.z.string().nullish(),
  created: import_zod37.z.number().nullish(),
  model: import_zod37.z.string().nullish(),
  choices: import_zod37.z.array(
    import_zod37.z.object({
      delta: import_zod37.z.object({
        role: import_zod37.z.enum(["assistant"]).optional(),
        content: mistralContentSchema,
        tool_calls: import_zod37.z.array(
          import_zod37.z.object({
            id: import_zod37.z.string(),
...
var MistralTextEmbeddingResponseSchema = import_zod39.z.object({
  data: import_zod39.z.array(import_zod39.z.object({ embedding: import_zod39.z.array(import_zod39.z.number()) })),
  usage: import_zod39.z.object({ prompt_tokens: import_zod39.z.number() }).nullish()
});
...
var deepSeekUsageSchema = import_zod40.z.object({
  prompt_cache_hit_tokens: import_zod40.z.number().nullish(),
  prompt_cache_miss_tokens: import_zod40.z.number().nullish()
});
var deepSeekResponseSchema = import_zod40.z.object({
  usage: deepSeekUsageSchema.nullish()
});
var deepSeekStreamChunkSchema = import_zod40.z.object({
...
var perplexityUsageSchema = import_zod41.z.object({
  prompt_tokens: import_zod41.z.number(),
  completion_tokens: import_zod41.z.number(),
  citation_tokens: import_zod41.z.number().nullish(),
  num_search_queries: import_zod41.z.number().nullish()
});
var perplexityImageSchema = import_zod41.z.object({
  image_url: import_zod41.z.string(),
  origin_url: import_zod41.z.string(),
  height: import_zod41.z.number(),
  width: import_zod41.z.number()
});
var perplexityResponseSchema = import_zod41.z.object({
  id: import_zod41.z.string(),
  created: import_zod41.z.number(),
  model: import_zod41.z.string(),
  choices: import_zod41.z.array(
    import_zod41.z.object({
      message: import_zod41.z.object({
        role: import_zod41.z.literal("assistant"),
        content: import_zod41.z.string()
      }),
      finish_reason: import_zod41.z.string().nullish()
    })
  ),
  citations: import_zod41.z.array(import_zod41.z.string()).nullish(),
  images: import_zod41.z.array(perplexityImageSchema).nullish(),
  usage: perplexityUsageSchema.nullish()
});
var perplexityChunkSchema = import_zod41.z.object({
  id: import_zod41.z.string(),
  created: import_zod41.z.number(),
  model: import_zod41.z.string(),
  choices: import_zod41.z.array(
    import_zod41.z.object({
      delta: import_zod41.z.object({
        role: import_zod41.z.literal("assistant"),
        content: import_zod41.z.string()
      }),
      finish_reason: import_zod41.z.string().nullish()
...
var perplexityErrorSchema = import_zod41.z.object({
  error: import_zod41.z.object({
    code: import_zod41.z.number(),
    message: import_zod41.z.string().nullish(),
    type: import_zod41.z.string().nullish()
  })
});
...
var ollamaErrorDataSchema = import_zod43.z.object({
  error: import_zod43.z.object({
    code: import_zod43.z.string().nullable(),
    message: import_zod43.z.string(),
    param: import_zod43.z.any().nullable(),
...
var ollamaChatResponseSchema = import_zod42.z.object({
  created_at: import_zod42.z.string(),
  done: import_zod42.z.literal(true),
  done_reason: import_zod42.z.string().optional().nullable(),
  eval_count: import_zod42.z.number(),
  eval_duration: import_zod42.z.number(),
  load_duration: import_zod42.z.number().optional(),
  message: import_zod42.z.object({
    content: import_zod42.z.string(),
    role: import_zod42.z.string(),
    tool_calls: import_zod42.z.array(
      import_zod42.z.object({
        function: import_zod42.z.object({
          arguments: import_zod42.z.record(import_zod42.z.any()),
          name: import_zod42.z.string()
        }),
        id: import_zod42.z.string().optional()
      })
    ).optional().nullable()
  }),
  model: import_zod42.z.string(),
  prompt_eval_count: import_zod42.z.number().optional(),
  prompt_eval_duration: import_zod42.z.number().optional(),
  total_duration: import_zod42.z.number()
});
var ollamaChatStreamChunkSchema = import_zod42.z.discriminatedUnion("done", [
  import_zod42.z.object({
    created_at: import_zod42.z.string(),
    done: import_zod42.z.literal(false),
    message: import_zod42.z.object({
      content: import_zod42.z.string(),
      role: import_zod42.z.string()
    }),
...
var ollamaTextEmbeddingResponseSchema = import_zod44.z.object({
  embeddings: import_zod44.z.array(import_zod44.z.array(import_zod44.z.number())),
  prompt_eval_count: import_zod44.z.number().nullable()
});
